...
bool LAnim::serverProcessProjectileAnimState(int x, int y, int i ,int j ,int renderSpeed)
{
	if (animTimer[i][j].getTicks() > renderSpeed / tickCount)
	{
		currentTickClient[i][j]++;
		animTimer[i][j].reset();
	}
	if (currentTickClient[i][j] >= tickCount - 2)
	{
		animTimer[i][j].reset();
...
void LAnim::renderTexture(SDL_Renderer* gRenderer, int x, int y, float ang, int i, int j, bool inverse, SDL_RendererFlip flip, int renderSpeed, int renderInverseSpeed)
{
	if (!isInverseSeq[i][j])
	{
		seqInUse[i][j] = true;

		animTexture[currentTickClient[i][j]].render(gRenderer, x, y, NULL, ang, NULL, flip);
	...
	else if (inverse && isInverseSeq[i][j])
	{
		animTexture[tickCount - currentTickClient[i][j]].render(gRenderer, x, y, NULL, ang, NULL, flip);

		if (animTimer[i][j].getTicks() > renderInverseSpeed / tickCount)
		{
			currentTickClient[i][j]++;
			animTimer[i][j].reset();
		}
...
void LAnim::renderStaticAnim(SDL_Renderer* gRenderer)
{
	//RENDER SEQ ANIM
	...
	if (!p_isCrop)
	{
		for (unsigned int i = 0; i < MAX_VARIABLE_AMMOUNT; i++)
		{
			for (unsigned int j = 0; j < MAX_VARIABLE_AMMOUNT; j++)
			{
				...
				{
					animTexture[currentTickClient[i][j]].render(gRenderer, seqPosX[i][j], seqPosY[i][j], NULL, animAngle[i][j], NULL, SDL_FLIP_NONE);
				...
				{
					animTexture[tickCount - currentTickClient[i][j]].render(gRenderer, seqPosX[i][j], seqPosY[i][j], NULL, animAngle[i][j], NULL, SDL_FLIP_NONE);
	...
	else
	{
		...
		for (unsigned int i = 0; i < MAX_VARIABLE_AMMOUNT; i++)
		{
			...
			{
				animTexture[0].render(gRenderer, cropPosX[i], cropPosY[i], &animRect[currentTickClient[0][i]], 0, 0, SDL_FLIP_NONE);

				if (animTimer[0][i].getTicks() > 1000 / tickCount)
				{
			...
			{
				animTexture[0].render(gRenderer, cropPosX[i], cropPosY[i], &animRect[tickCount - currentTickClient[0][i]], 0, 0, SDL_FLIP_NONE);
